...
use super::{
    config::{GenericReplayBufferConfig, PerConfig},
    BatchBase, GenericTransitionBatch,
};
use crate::iw_scheduler::IwScheduler;
use crate::sum_tree::SumTree;
use anyhow::Result;
use border_core::{ExperienceBuffer, ReplayBuffer, TransitionBatch};
...
///
/// ```ignore
/// let config = GenericReplayBufferConfig {
///     capacity: 10000,
...
/// };
///
/// let mut buffer = GenericReplayBuffer::<Tensor, Tensor>::build(&config);
///
/// // Add transitions
...
/// ```
pub struct GenericReplayBuffer<O, A>
where
    O: BatchBase,
    A: BatchBase,
{
...
impl<O, A> GenericReplayBuffer<O, A>
where
    O: BatchBase,
    A: BatchBase,
{
...
impl<O, A> ExperienceBuffer for GenericReplayBuffer<O, A>
where
    O: BatchBase,
    A: BatchBase,
{
...
impl<O, A> ReplayBuffer for GenericReplayBuffer<O, A>
where
    O: BatchBase,
    A: BatchBase,
{
    type Config = GenericReplayBufferConfig;
